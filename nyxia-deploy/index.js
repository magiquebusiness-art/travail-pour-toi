...
__export(notifier_exports, {
  notify: /* @__PURE__ */ __name(() => notify, "notify"),
  sendDiscord: /* @__PURE__ */ __name(() => sendDiscord, "sendDiscord"),
  sendEmail: /* @__PURE__ */ __name(() => sendEmail, "sendEmail")
});
...
async function sendEmail(toEmail, fromEmail, event) {
  ...
  const icon = ICONS[event.type] || "\u2022";
  const title2 = `${icon} ${event.title || event.type}`;
  const html = buildEmailHtml(title2, event);
  const text = buildEmailText(title2, event);
  const payload = {
    personalizations: [{ to: [{ email: toEmail }] }],
    from: { email: fromEmail, name: "NyXia \xB7 Publication-Web" },
    subject: `NyXia \xB7 ${title2}`,
    content: [
...
function buildEmailHtml(title2, event) {
  const colorMap = {
  ...
  <div style="max-width:600px;margin:32px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08)">
    <div style="background:${accent};padding:24px 28px">
      <div style="color:#fff;font-size:22px;font-weight:700">${title2}</div>
...
function buildEmailText(title2, event) {
  const lines = [title2, ""];
...
async function notify(config2, event) {
  const results = {};
  const promises = [];
  if (config2.discordWebhook) {
    promises.push(
      sendDiscord(config2.discordWebhook, event).then((r) => {
        results.discord = r;
      })
    );
  }
  if (config2.emailTo && config2.emailFrom) {
    promises.push(
      sendEmail(config2.emailTo, config2.emailFrom, event).then((r) => {
        results.email = r;
      })
    );
  }
...
__export(sandbox_exports, {
  cfDryRun: /* @__PURE__ */ __name(() => cfDryRun, "cfDryRun"),
  formatSandboxReport: /* @__PURE__ */ __name(() => formatSandboxReport, "formatSandboxReport"),
  runSandbox: /* @__PURE__ */ __name(() => runSandbox, "runSandbox"),
  runSandboxMultiple: /* @__PURE__ */ __name(() => runSandboxMultiple, "runSandboxMultiple")
});
function analyzeCode(code, fileType = "javascript") {
  ...
  const getLine = /* @__PURE__ */ __name2((index) => {
    let count3 = 0;
    for (let i = 0; i < lines.length; i++) {
      count3 += lines[i].length + 1;
      if (count3 > index) return i + 1;
    }
    return lines.length;
  }, "getLine");
...
async function runSandbox(options) {
  ...
  report2.issues = issues;
  report2.metrics = metrics;
  report2.score = computeScore(issues);
  const scoreInfo = getScoreLabel(report2.score);
  report2.scoreLabel = scoreInfo.label;
  report2.canDeploy = scoreInfo.deploy;
  if (!skipCfDryRun && cfToken && cfAccountId && workerName && fileType === "worker") {
    report2.dryRun = await cfDryRun(cfToken, cfAccountId, workerName, code);
    if (!report2.dryRun.success) {
      report2.canDeploy = false;
      report2.score = Math.min(report2.score, 40);
    }
  }
  ...
  report2.passed = report2.score >= 70 && criticals === 0 && errors === 0;
  const parts = [];
  ...
  report2.summary = report2.passed ? `\u2713 ${filename} valid\xE9 (score: ${report2.score}/100)${parts.length ? " \u2014 " + parts.join(", ") : ""}` : `\u2717 ${filename} bloqu\xE9 (score: ${report2.score}/100) \u2014 ${parts.join(", ")}`;
  return report2;
}
__name(runSandbox, "runSandbox");
function formatSandboxReport(report2) {
  const lines = [];
  const scoreBar = "\u2588".repeat(Math.floor(report2.score / 10)) + "\u2591".repeat(10 - Math.floor(report2.score / 10));
  lines.push(`**Sandbox \u2014 \`${report2.filename}\`**`);
  lines.push(`Score : \`${scoreBar}\` ${report2.score}/100 \u2014 ${report2.scoreLabel}`);
  lines.push(`Taille : ${report2.metrics.lines} lignes \xB7 ${report2.metrics.functions} fonctions \xB7 ${report2.metrics.imports} imports`);
  lines.push("");
  ...
  if (report2.dryRun) {
    lines.push("");
    lines.push(
      report2.dryRun.success ? "\u2713 **Dry-run Cloudflare** \u2014 script accept\xE9" : `\u2717 **Dry-run Cloudflare \xE9chou\xE9** \u2014 ${report2.dryRun.errors?.[0]?.message || report2.dryRun.error}`
    );
  }
  lines.push("");
  lines.push(
    report2.canDeploy ? "\u2705 **Pr\xEAt \xE0 d\xE9ployer**" : "\u{1F6AB} **D\xE9ploiement bloqu\xE9** \u2014 corrige les probl\xE8mes ci-dessus d'abord"
  );
  return lines.join("\n");
}
...
async function runSandboxMultiple(files, cfCredentials = {}) {
  ...
  for (const file of files) {
    const report2 = await runSandbox({
      code: file.content,
    ...
    results.push(report2);
    globalScore += report2.score;
  }
...
var CF_FORBIDDEN_APIS;
var DANGEROUS_PATTERNS;
var BEST_PRACTICES;
var CF_WORKER_CHECKS;
var init_sandbox = __esm({
  "sandbox.js"() {
    ...
    __name2(analyzeCode, "analyzeCode");
    __name2(checkBasicSyntax, "checkBasicSyntax");
    __name2(computeScore, "computeScore");
    __name2(getScoreLabel, "getScoreLabel");
    __name2(cfDryRun, "cfDryRun");
    __name2(runSandbox, "runSandbox");
    __name2(formatSandboxReport, "formatSandboxReport");
    __name2(runSandboxMultiple, "runSandboxMultiple");
    __name2(detectFileType, "detectFileType");
  }
});
var site_generator_exports = {};
__export(site_generator_exports, {
  PALETTES: /* @__PURE__ */ __name(() => PALETTES, "PALETTES"),
  deploySiteToPages: /* @__PURE__ */ __name(() => deploySiteToPages, "deploySiteToPages"),
  generateSite: /* @__PURE__ */ __name(() => generateSite, "generateSite"),
  registerAsAffiliate: /* @__PURE__ */ __name(() => registerAsAffiliate, "registerAsAffiliate"),
  resolveSubdomain: /* @__PURE__ */ __name(() => resolveSubdomain, "resolveSubdomain"),
  runSiteGenerationPipeline: /* @__PURE__ */ __name(() => runSiteGenerationPipeline, "runSiteGenerationPipeline")
});
...
async function runSiteGenerationPipeline(options, env2) {
  const {
    type,
    prompt,
    language,
  ...
  const report2 = { steps: [], success: false, startedAt: (/* @__PURE__ */ new Date()).toISOString() };
  const step = /* @__PURE__ */ __name2((name, status, data = {}) => report2.steps.push({ step: name, status, ...data, at: (/* @__PURE__ */ new Date()).toISOString() }), "step");
  try {
    step("generate", "running");
    const { html } = await generateSite(env2.OPENROUTER_API_KEY, {
      type,
      prompt,
      language,
      palette,
    ...
    report2.html = html;
    report2.subdomain = subdomainInfo;
  } catch (err) {
    step("pipeline", "error", { error: err.message });
    report2.error = err.message;
  }
  return report2;
}
...
Design : moderne, e-commerce professionnel. Inspir\xE9 Shopify.`
    ...
    BASE_CSS = /* @__PURE__ */ __name2((palette) => `
...
`, "BASE_CSS");
    ...
    __name2(generateSite, "generateSite");
    __name2(resolveSubdomain, "resolveSubdomain");
    __name2(deploySiteToPages, "deploySiteToPages");
    __name2(registerAsAffiliate, "registerAsAffiliate");
    __name2(runSiteGenerationPipeline, "runSiteGenerationPipeline");
  }
});
var payments_exports = {};
__export(payments_exports, {
  PAYMENT_LINKS: /* @__PURE__ */ __name(() => PAYMENT_LINKS, "PAYMENT_LINKS"),
  activatePlan: /* @__PURE__ */ __name(() => activatePlan, "activatePlan"),
  getPaymentUrl: /* @__PURE__ */ __name(() => getPaymentUrl, "getPaymentUrl"),
  getPlanPaymentInfo: /* @__PURE__ */ __name(() => getPlanPaymentInfo, "getPlanPaymentInfo")
});
  ...
  "payments.js"() {
    PAYMENT_LINKS = {
      pro: {
        ...
        urlWithRef: /* @__PURE__ */ __name2((refId) => `https://ton-compte.systeme.io/pro-mensuel?ref=${refId}`, "urlWithRef"),
      ...
      visionnaire: {
        ...
        urlWithRef: /* @__PURE__ */ __name2((refId) => `https://ton-compte.systeme.io/visionnaire-mensuel?ref=${refId}`, "urlWithRef"),
      ...
      meta: {
        ...
        urlWithRef: /* @__PURE__ */ __name2((refId) => `https://www.publication-web.com/nyxiawebsite/97?ref=${refId}`, "urlWithRef"),
        systemeioTag: "meta_presence_achat"
      }
    };
    __name2(getPaymentUrl, "getPaymentUrl");
    __name2(activatePlan, "activatePlan");
    __name2(getPlanPaymentInfo, "getPlanPaymentInfo");
  }
});
var plans_exports = {};
__export(plans_exports, {
  FREE_EMAIL_SEQUENCE: /* @__PURE__ */ __name(() => FREE_EMAIL_SEQUENCE, "FREE_EMAIL_SEQUENCE"),
  PLANS: /* @__PURE__ */ __name(() => PLANS, "PLANS"),
  addWatermark: /* @__PURE__ */ __name(() => addWatermark, "addWatermark"),
  canDo: /* @__PURE__ */ __name(() => canDo, "canDo"),
  getPlan: /* @__PURE__ */ __name(() => getPlan, "getPlan"),
  plansForDisplay: /* @__PURE__ */ __name(() => plansForDisplay, "plansForDisplay"),
  registerFreeLead: /* @__PURE__ */ __name(() => registerFreeLead, "registerFreeLead")
});
...
var init_plans = __esm({
  "plans.js"() {
    ...
    __name2(addWatermark, "addWatermark");
    FREE_EMAIL_SEQUENCE = [
      {
        day: 0,
    ...
    __name2(registerFreeLead, "registerFreeLead");
    __name2(plansForDisplay, "plansForDisplay");
  }
});
...
__name2(listProjects, "listProjects");
...
__name2(resolveCredentials, "resolveCredentials");
...
__name2(stateSummary, "stateSummary");
var wait = /* @__PURE__ */ __name2((ms) => new Promise((r) => setTimeout(r, ms)), "wait");
...
__name2(githubPushMultipleFiles, "githubPushMultipleFiles");
...
async function runDeployPipeline(options) {
  ...
  const step = /* @__PURE__ */ __name2((name, status, data = {}) => {
    report2.steps.push({ step: name, status, ...data, at: (/* @__PURE__ */ new Date()).toISOString() });
  }, "step");
  try {
    if (!skipGithub && github) {
      ...
      try {
        ...
        report2.error = `GitHub push \xE9chou\xE9 : ${err.message}`;
        return report2;
      }
    }
    if (cloudflare) {
      ...
      try {
        ...
        if (j.success) {
          ...
          report2.error = `D\xE9ploiement Cloudflare \xE9chou\xE9`;
          return report2;
        }
      } catch (err) {
    ...
    report2.success = true;
    report2.completedAt = (/* @__PURE__ */ new Date()).toISOString();
  } catch (err) {
    report2.error = err.message;
    step("pipeline", "error", { error: err.message });
  }
...
__name2(runDeployPipeline, "runDeployPipeline");
function formatReport(report2) {
  ...
  lines.push("");
  lines.push(report2.success ? "\u2713 Pipeline termin\xE9 avec succ\xE8s." : `\u2717 Pipeline interrompu : ${report2.error || "erreur inconnue"}`);
  return lines.join("\n");
}
__name(formatReport, "formatReport");
__name2(formatReport, "formatReport");
var KEYS = {
...
__name(encrypt2, "encrypt2");
__name2(encrypt2, "encrypt");
...
__name(decrypt2, "decrypt2");
__name2(decrypt2, "decrypt");
...
__name(kvRead2, "kvRead2");
__name2(kvRead2, "kvRead");
async function kvWrite2(kv, key, data, secret) {
  await kv.put(key, await encrypt2(data, secret));
}
__name(kvWrite2, "kvWrite2");
__name2(kvWrite2, "kvWrite");
...
__name2(getRecentSessions, "getRecentSessions");
...
__name2(logProjectEvent, "logProjectEvent");
...
__name2(getProjectHistory, "getProjectHistory");
...
__name2(getAllProjectsActivity, "getAllProjectsActivity");
function buildMemoryContext() {
  ...
  const lines = [];
  if (profile3?.name) {
    lines.push(`UTILISATEUR : ${profile3.name}`);
    if (profile3.preferences?.language) lines.push(`  Langue pr\xE9f\xE9r\xE9e : ${profile3.preferences.language}`);
    if (profile3.preferences?.codeStyle) lines.push(`  Style de code   : ${profile3.preferences.codeStyle}`);
    if (profile3.style?.tone) lines.push(`  Ton souhait\xE9    : ${profile3.style.tone}`);
...
__name2(buildMemoryContext, "buildMemoryContext");
...
__name2(generateSessionSummary, "generateSessionSummary");
init_notifier();
...
__name2(verifySystemeioWebhook, "verifySystemeioWebhook");
...
__name2(parseSystemeioEvent, "parseSystemeioEvent");
...
__name2(getIntegrationGuide, "getIntegrationGuide");
var SCHEMAS = {
  affiliationpro: {
...
__name2(generateD1WorkerCode, "generateD1WorkerCode");
...
__name2(generateWranglerD1Config, "generateWranglerD1Config");
var CSS_PRESETS = {
  // ── Boutons ──────────────────────────────────────────────
  button_glow: {
...
var EMAIL_TEMPLATES = {
  welcome: {
    ...
    html: /* @__PURE__ */ __name2((vars = {}) => `<!DOCTYPE html>
...
__name2(generateProjectCSS, "generateProjectCSS");
init_sandbox();
init_site_generator();
var TYPE_KEYWORDS = {
...
__name2(classifyImageRequest, "classifyImageRequest");
...
__name2(shouldUseCfAI, "shouldUseCfAI");
var CF_AI_STYLES = {
...
async function generateWithCfAI(env2, options) {
  const {
    description,
  ...
  if (!env2.AI) throw new Error("Binding AI manquant \u2014 ajoute [ai] dans wrangler.toml");
  ...
  try {
    const response = await env2.AI.run("@cf/stabilityai/stable-diffusion-xl-base-1.0", {
      prompt,
...
__name2(generateWithCfAI, "generateWithCfAI");
...
${tpl.suffixes}`;
  ...
  return {
    ...
    instructions,
    estimatedCost: platform2 === "midjourney" ? "~0.02$" : platform2 === "leonardo" ? "~3 cr\xE9dits" : "~0.04$",
...
__name2(generateMidjourneyPrompt, "generateMidjourneyPrompt");
...
__name2(generatePlaceholder, "generatePlaceholder");
...
__name2(generatePatternSVG, "generatePatternSVG");
async function processImageRequest(env2, options) {
  const {
    description,
  ...
  const useCfAI = !returnPromptOnly && shouldUseCfAI(imageType) && !!env2?.AI;
  if (useCfAI) {
    const result = await generateWithCfAI(env2, {
      description,
      imageType,
      palette,
...
__name2(processImageRequest, "processImageRequest");
...
__name2(buildPaletteHint, "buildPaletteHint");
...
__name2(buildPaletteDesc, "buildPaletteDesc");
...
__name2(classifyRequest, "classifyRequest");
async function callAgent(llmKey, agentId, task, context2 = {}) {
  const systemPrompt = AGENT_PROMPTS[agentId];
  if (!systemPrompt) throw new Error(`Agent inconnu : ${agentId}`);
  const contextStr = Object.keys(context2).length > 0 ? `
...
async function orchestrate(llmKey, masterTask, context2 = {}) {
...
Contexte : ${JSON.stringify(context2)}`
      }]
    })
  });
  let tasks = [];
  ...
  const agentPromises = tasks.map(async ({ agent, task }) => {
    try {
      const result = await callAgent(llmKey, agent, task, context2);
...
__name2(orchestrate, "orchestrate");
async function agentCopywriter(llmKey, options) {
  const { type, context: context2, tone = "professionnel", audience, product, language = "fr" } = options;
  const taskMap = {
  ...
  const task = taskMap[type] || options.customTask || `R\xE9dige du contenu pour : ${context2}`;
  return callAgent(llmKey, "copywriter", task, { language, tone, audience, product });
}
__name(agentCopywriter, "agentCopywriter");
__name2(agentCopywriter, "agentCopywriter");
...
__name2(agentCommunity, "agentCommunity");
...
__name2(agentAnalyst, "agentAnalyst");
...
__name2(agentSupport, "agentSupport");
var AGENT_TOOLS = [
  {
    type: "function",
    function: {
...
__name2(isProtectedFile, "isProtectedFile");
...
__name2(detectImprovements, "detectImprovements");
...
__name2(generatePatch, "generatePatch");
...
__name2(generateDiff, "generateDiff");
async function runSelfImprovementPipeline(options, env2) {
  const {
    improvementId,
    improvement,
    currentCode,
  ...
  try {
    ...
    const patch = await generatePatch(env2.OPENROUTER_API_KEY, { improvement, currentCode, filepath });
    ...
    if (env2.GITHUB_TOKEN && improvement.github_repo) {
      step("backup", "running");
      try {
        ...
        report2.backupBranch = backupBranch;
      } catch (err) {
    ...
    try {
      const workerUrl = env2.WORKER_URL || "https://nyxia-agent.workers.dev";
      ...
      if (report2.backupBranch) {
        step("rollback", "success", {
          branch: report2.backupBranch,
          message: `Code restaur\xE9 depuis ${report2.backupBranch}`
        });
        report2.rollbackDone = true;
      } else {
        step("rollback", "warning", { message: "Rollback manuel requis \u2014 pas de branche backup" });
      }
      report2.error = "Health check \xE9chou\xE9 apr\xE8s modification \u2014 rollback effectu\xE9";
      return report2;
    }
    ...
    const logEntry = {
      ...
      backupBranch: report2.backupBranch || null
    };
    if (env2.NYXIA_VAULT) {
      await env2.NYXIA_VAULT.put(
        `improvement:${report2.id}`,
    ...
    if (env2.NYXIA_VAULT) {
      const discordWebhook = await env2.NYXIA_VAULT.get("nyxia:notify:discord").catch(() => null);
      if (discordWebhook) {
...
__name2(runSelfImprovementPipeline, "runSelfImprovementPipeline");
...
__name2(getImprovementHistory, "getImprovementHistory");
function formatImprovementReport(report2) {
  const lines = [];
  const icons = { running: "\u27F3", success: "\u2713", blocked: "\u{1F512}", error: "\u2717", warning: "\u26A0", skipped: "\u25CB", failed: "\u2717" };
  lines.push(`**Auto-am\xE9lioration : ${report2.improvement?.title || report2.id}**`);
  lines.push(`Fichier : \`${report2.filepath}\``);
  lines.push("");
  if (report2.blocked) {
    lines.push(`\u{1F6AB} **BLOQU\xC9E** \u2014 ${report2.steps.find((s) => s.status === "blocked")?.message || "Raison inconnue"}`);
    return lines.join("\n");
  }
  report2.steps.forEach((s) => {
    const icon = icons[s.status] || "\u25CB";
    lines.push(`${icon} **${s.step}** \u2014 ${s.message || s.status}${s.score ? ` (${s.score}/100)` : ""}`);
  });
  lines.push("");
  if (report2.rollbackDone) {
    lines.push("\u26A0 **Rollback effectu\xE9** \u2014 code restaur\xE9 depuis le backup");
  } else if (report2.success) {
    lines.push(`\u2705 **Am\xE9lioration appliqu\xE9e avec succ\xE8s**`);
    if (report2.backupBranch) lines.push(`  Backup disponible sur : \`${report2.backupBranch}\``);
  } else {
    lines.push(`\u274C **\xC9chec** \u2014 ${report2.error}`);
  }
  return lines.join("\n");
}
__name(formatImprovementReport, "formatImprovementReport");
__name2(formatImprovementReport, "formatImprovementReport");
  ...
  try {
    switch (name) {
      ...
      case "apply_improvement": {
        ...
        const report2 = await runSelfImprovementPipeline({
        ...
        return {
          ...
          rollback_done: report2.rollbackDone,
          report: formatImprovementReport(report2),
          steps: report2.steps,
          new_code: report2.success ? report2.newCode?.slice(0, 200) + "..." : null,
          backup_branch: report2.backupBranch,
          message: report2.success ? `\u2705 Am\xE9lioration appliqu\xE9e : ${args.improvement_title}` : report2.blocked ? `\u{1F512} Bloqu\xE9 : ${report2.steps.find((s) => s.status === "blocked")?.message}` : `\u274C \xC9chec : ${report2.error}`
        };
      }
      ...
      case "orchestrate": {
        const groqKey = process.env?.GROQ_API_KEY || "";
        if (!groqKey) return { success: false, error: "GROQ_API_KEY requise" };
        const report2 = await orchestrate(groqKey, args.task, args.context || {});
        return {
          success: report2.success,
          agents_used: report2.agentsUsed,
          tasks_count: report2.tasksCount,
          final_answer: report2.finalAnswer,
          details: report2.results.map((r) => ({
            agent: r.agent,
            type: r.type,
            tokens: r.tokens,
          ...
          message: `Orchestration : ${report2.agentsUsed.join("+")} \u2192 ${report2.tasksCount} t\xE2che(s)`
        };
      }
      ...
      case "generate_site_full": {
        ...
        const report2 = await runSiteGenerationPipeline({
          type: args.type,
          prompt: args.prompt,
        ...
        const stepsOk = report2.steps.filter((s) => s.status === "success").length;
        return {
          success: report2.success,
          site_url: report2.siteUrl,
          affiliate_link: report2.affiliateLink,
          dashboard: report2.affiliateDashboard,
          subdomain: report2.subdomain,
          html_preview: report2.html?.slice(0, 500) + "...",
          html_length: report2.html?.length,
          steps: report2.steps.map((s) => `${s.status === "success" ? "\u2713" : s.status === "warning" ? "\u26A0" : "\u2717"} ${s.step}${s.url ? " \u2192 " + s.url : s.error ? " : " + s.error : ""}`),
          message: report2.success ? `\u2705 Site "${args.product_name || args.type}" g\xE9n\xE9r\xE9 et d\xE9ploy\xE9${report2.siteUrl ? " \u2192 " + report2.siteUrl : ""}` : `\u26A0 Pipeline partiel (${stepsOk}/${report2.steps.length} \xE9tapes) : ${report2.error || "voir steps"}`
        };
      }
      ...
      case "resolve_subdomain": {
        const info3 = resolveSubdomain({ clientDomain: args.client_domain, clientSlug: args.client_slug });
        return { success: true, ...info3 };
      }
      case "list_palettes":
      ...
      case "sandbox_test_single": {
        ...
        return {
          success: report2.passed,
          can_deploy: report2.canDeploy,
          score: report2.score,
          score_label: report2.scoreLabel,
          report: formatSandboxReport(report2),
          issues: report2.issues,
          metrics: report2.metrics,
          message: report2.summary
        };
      }
      // ── Phase 7 : D1 & Designer ──────────────────────────
      case "d1_query":
      ...
      // ── Phase 5 : Mémoire ─────────────────────────────────
      case "update_profile": {
        const profile3 = await updateProfile(kv, secret, {
          name: args.name,
          notes: args.notes,
          preferences: args.preferences,
          style: args.style
        });
        return { success: true, message: `Profil mis \xE0 jour${args.name ? ` \u2014 Bonjour ${args.name} !` : ""}`, profile: profile3 };
      }
...
function buildContextualDescription(imageType, context2, role, gender) {
  const g = gender === "man" ? "man" : "woman";
  const r = role || "entrepreneur";
  switch (imageType) {
    case "person":
      return `${g} ${r}, ${context2}, professional`;
    case "product":
      return `digital product mockup for ${context2}`;
    case "background":
      return `abstract professional background for ${context2} website`;
    case "banner":
      return `wide hero banner for ${context2}, professional, inspiring`;
    case "illustration":
      return `icon illustration for ${context2}, flat design`;
    default:
      return `professional image for ${context2}`;
  }
}
__name(buildContextualDescription, "buildContextualDescription");
__name2(buildContextualDescription, "buildContextualDescription");
init_notifier();
...
__name2(createSignedToken, "createSignedToken");
...
__name2(verifySignedToken, "verifySignedToken");
...
__name2(buildClientPrompt, "buildClientPrompt");
...
__name2(callOpenRouterClient, "callOpenRouterClient");
...
__name2(callOpenRouter, "callOpenRouter");
async function fireNotif(env2, type, title2, description, extra = {}) {
  try {
    const notifConfig = {
      discordWebhook: env2.NYXIA_VAULT ? await env2.NYXIA_VAULT.get("nyxia:notify:discord").catch(() => null) : null,
      emailTo: env2.NYXIA_VAULT ? await env2.NYXIA_VAULT.get("nyxia:notify:email_to").catch(() => null) : null,
      emailFrom: env2.NYXIA_VAULT ? await env2.NYXIA_VAULT.get("nyxia:notify:email_from").catch(() => null) : null
    };
    if (notifConfig.discordWebhook || notifConfig.emailTo) {
      await notify(notifConfig, { type, title: title2, description, ...extra }).catch(() => {
      });
    }
  } catch {
  }
...
var index_default = {
  async fetch(request, env2) {
    ...
    if (env2.NYXIA_VAULT && env2.VAULT_SECRET) {
      await Promise.all([
        loadVault(env2.NYXIA_VAULT, env2.VAULT_SECRET),
        loadMemory(env2.NYXIA_VAULT, env2.VAULT_SECRET)
      ]);
    }
    if (url.pathname === "/api/auth/login" && request.method === "POST") {
      try {
        ...
        if (!env2.NYXIA_VAULT) return Response.json({ success: false, message: "Service indisponible" }, { status: 503, headers: cors });
        const accountRaw = await env2.NYXIA_VAULT.get(`account:${email.toLowerCase()}`).catch(() => null);
        if (!accountRaw) return Response.json({ success: false, message: "Identifiants incorrects" }, { status: 401, headers: cors });
        const account = JSON.parse(accountRaw);
        const encoder = new TextEncoder();
        const keyData = encoder.encode(env2.VAULT_SECRET);
        ...
        const tokenStr = await createSignedToken(tokenData, env2.VAULT_SECRET);
        await env2.NYXIA_VAULT.put(
    ...
    if (url.pathname === "/api/auth/register" && request.method === "POST") {
      try {
        ...
        if (!env2.NYXIA_VAULT) return Response.json({ success: false, message: "Service indisponible" }, { status: 503, headers: cors });
        const existing = await env2.NYXIA_VAULT.get(`account:${email.toLowerCase()}`).catch(() => null);
        ...
        await env2.NYXIA_VAULT.put(`account:${email.toLowerCase()}`, JSON.stringify(account), { expirationTtl: 365 * 24 * 60 * 60 });
        await env2.NYXIA_VAULT.put(`account:id:${id}`, email.toLowerCase(), { expirationTtl: 365 * 24 * 60 * 60 });
    ...
    if (url.pathname === "/webhook/systemeio" && request.method === "POST") {
      try {
        const webhookSecret = env2.SYSTEMEIO_SECRET || "";
        ...
        const title2 = event.eventType === "order.completed" ? `Vente \u2014 ${event.product} (CA$${event.amount})` : event.eventType === "subscription.created" ? `Abonn\xE9 \u2014 ${event.contactEmail || "?"}` : `\xC9v\xE9nement : ${event.eventType}`;
        fireNotif(env2, "systemeio_webhook", title2, event.contactEmail || "").catch(() => {
        });
    ...
    if (url.pathname === "/api/client-chat" && request.method === "POST") {
      try {
        ...
        if (!env2.OPENROUTER_API_KEY) return Response.json({ error: "Service indisponible" }, { status: 500, headers: cors });
        const authUser = await requireAuth(request, env2.VAULT_SECRET || "");
        const userPlan = authUser?.plan || "ambassadeur";
        const userName = authUser?.name || null;
        const response = await callOpenRouterClient(env2.OPENROUTER_API_KEY, messages, userPlan, userName);
        return Response.json({
          role: "assistant",
    ...
    if (url.pathname === "/api/chat" && request.method === "POST") {
      ...
      try {
        const { messages } = await request.json();
        if (!Array.isArray(messages)) return Response.json({ error: "messages[] requis" }, { status: 400, headers: cors });
        if (!env2.OPENROUTER_API_KEY) return Response.json({ error: "OPENROUTER_API_KEY manquante" }, { status: 500, headers: cors });
        const systemPrompt = buildSystemPrompt(stateSummary(), buildMemoryContext());
        const fullMessages = [{ role: "system", content: systemPrompt }, ...messages];
        const response = await callOpenRouter(env2.OPENROUTER_API_KEY, fullMessages);
        ...
        for (const tc of message.tool_calls) {
          ...
          const result = await executeTool(name, args, env2.NYXIA_VAULT, env2.VAULT_SECRET);
        ...
        const finalResponse = await callOpenRouter(env2.OPENROUTER_API_KEY, [...fullMessages, message, ...toolMessages], false);
    ...
    if (url.pathname === "/api/save-session" && request.method === "POST") {
      ...
      try {
        ...
        if (!env2.NYXIA_VAULT || !env2.VAULT_SECRET) return Response.json({ error: "Vault non configur\xE9" }, { status: 500, headers: cors });
        const summary = await generateSessionSummary(env2.OPENROUTER_API_KEY, messages || []);
        const session = await saveSession(env2.NYXIA_VAULT, env2.VAULT_SECRET, summary);
        if (Array.isArray(projectEvents)) {
          for (const ev of projectEvents) {
            await logProjectEvent(env2.NYXIA_VAULT, env2.VAULT_SECRET, ev.project, ev);
          }
        }
    ...
    if (url.pathname === "/api/memory") {
      ...
      if (request.method === "POST") {
        try {
          ...
          const profile3 = await updateProfile(env2.NYXIA_VAULT, env2.VAULT_SECRET, updates);
          return Response.json({ success: true, profile: profile3 }, { headers: cors });
        } catch (err) {
          return Response.json({ error: err.message }, { status: 500, headers: cors });
        }
      }
    ...
    if (url.pathname === "/api/generate-site" && request.method === "POST") {
      ...
      try {
        ...
        const report2 = await runSiteGenerationPipeline2({
        ...
        if (plan.limits.watermark && report2.html) report2.html = addWatermark2(report2.html);
        if (planId === "free" && body.client_email && env2.NYXIA_VAULT) {
          await registerFreeLead2(env2.NYXIA_VAULT, {
        ...
        return Response.json({
          success: report2.success,
          site_url: report2.siteUrl,
          affiliate_link: canDo2(planId, "affiliation_active") ? report2.affiliateLink : null,
          dashboard: canDo2(planId, "affiliation_active") ? report2.affiliateDashboard : null,
          subdomain: report2.subdomain,
          plan: planId,
    ...
    if (url.pathname === "/api/backup" && request.method === "POST") {
      if (!user) return Response.json({ error: "Non autoris\xE9" }, { status: 401, headers: cors });
      try {
        if (!env2.NYXIA_VAULT) return Response.json({ error: "Vault non configur\xE9" }, { status: 500, headers: cors });
        const ts = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-");
        const backupId = `backup:${ts}`;
        const keys = [];
        let cursor = "";
        do {
          const list = await env2.NYXIA_VAULT.list({ prefix: "", cursor, limit: 100 });
          keys.push(...list.keys);
          cursor = list.cursor;
        ...
        for (const k of importantKeys) {
          const val = await env2.NYXIA_VAULT.get(k.name).catch(() => null);
        ...
        await env2.NYXIA_VAULT.put(backupId, backupPayload, { expirationTtl: 365 * 24 * 60 * 60 });
        return Response.json({
          success: true,
          backupId,
    ...
    if (url.pathname === "/api/backup/list" && request.method === "GET") {
      ...
      try {
        ...
        let cursor = "";
        do {
          const list = await env2.NYXIA_VAULT.list({ prefix: "backup:", cursor, limit: 100 });
          keys.push(...list.keys);
        ...
        for (const k of keys.sort((a, b) => b.name.localeCompare(a.name))) {
          const raw = await env2.NYXIA_VAULT.get(k.name).catch(() => null);
          if (raw) {
    ...
    if (url.pathname.startsWith("/api/backup/download/") && request.method === "GET") {
      ...
      try {
        ...
        if (!env2.NYXIA_VAULT) return Response.json({ error: "Vault non configur\xE9" }, { status: 500, headers: cors });
        const raw = await env2.NYXIA_VAULT.get(backupId);
    ...
    if (url.pathname === "/api/backup/restore" && request.method === "POST") {
      ...
      try {
        ...
        for (const [key, value] of Object.entries(backup.data)) {
          await env2.NYXIA_VAULT.put(key, value, { expirationTtl: 365 * 24 * 60 * 60 });
          restored++;
        }
        return Response.json({
          success: true,
    ...
    // ── Image aliases: map missing names to existing assets ──
    const _IMG_MAP = { "/nyxia-working.png": "/NyXia.png", "/nyxia-waiting.png": "/NyXia.png", "/nyxia-result.png": "/NyXia.png" };
    if (_IMG_MAP[url.pathname]) {
      const _au = new URL(_IMG_MAP[url.pathname], url.origin);
      return env2.ASSETS?.fetch(new Request(_au.toString(), request)) || new Response("Not found", { status: 404 });
    }
    return env2.ASSETS?.fetch(request) || new Response("NyXia Worker actif", { headers: cors });
  }
};
export {
