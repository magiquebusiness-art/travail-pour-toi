...
// GET /api/dashboard/students — List enrollments for this admin's formations
export async function GET(request: NextRequest) {
  try {
    ...
    if (formationId) {
      conditions.push('e.formation_id = ?')
    ...
    // Get formation list for filter dropdown
    const formations = await db.prepare(`
      SELECT f.id, f.title, COUNT(e.id) as student_count
      FROM formations f
      LEFT JOIN formation_enrollments e ON e.formation_id = f.id AND e.status = 'active'
      WHERE f.tenant_id = ?
      GROUP BY f.id
