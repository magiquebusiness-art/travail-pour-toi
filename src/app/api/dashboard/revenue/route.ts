...
// GET /api/dashboard/revenue — List payments for this admin's formations
export async function GET(request: NextRequest) {
  try {
    ...
    const monthSummary = await db.prepare(`
      SELECT COALESCE(SUM(CASE WHEN p.status = 'paid' THEN p.amount ELSE 0 END), 0) as month_revenue_cents
      FROM payments p
      INNER JOIN formations f ON f.id = p.formation_id
      WHERE f.tenant_id = ? AND p.paid_at >= ?
    `).bind(session.userId, startOfMonth.toISOString()).first()
    ...
    const weekSummary = await db.prepare(`
      ...
      INNER JOIN formations f ON f.id = p.formation_id
      WHERE f.tenant_id = ? AND p.paid_at >= ?
    `).bind(session.userId, startOfWeek.toISOString()).first()
