...
export default function NyXiaChatWidget() {
  ...
  async function send() {
    ...
    setIsTyping(true)
    try {
      const res = await fetch('/api/nyxia-chat', {
        method: 'POST',
        ...
        <div id="nx-messages">
          {nxFirst && (
            <div className="nx-welcome">
              ...
              <p>Je suis l&apos;agente IA de NyXia MarketPlace. Pose-moi une question ou dis-moi ce que tu cherches. 💜</p>
              <div className="nx-chips">
                ...
                <div className="nx-chip" onClick={() => { setInput("J'ai un produit à vendre sur le marketplace"); }}>J&apos;ai un produit</div>
              </div>
            </div>
          )}
          {nxMsgs.map((msg, i) => (
